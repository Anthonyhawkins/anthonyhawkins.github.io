...
| Week | 🔢 Math (1.5 hr/day) | 💻 C++ (1 hr/day) | 🛰️ Space Networking (1.5 hr/day) |
|------|----------------------|--------------------|------------------------------------|
| 1 | Algebra 2: polynomials, rational expressions | Variables, types, control flow, functions | Read: CCSDS 130.0-G-4 (protocol overview — it's a map, no math needed) |
| 2 | Algebra 2: exponentials & logarithms | Arrays, strings, pointers, references | **dB arithmetic unlocked** — learn dB, dBW, dBm conversions (just log₁₀) |
| 3 | Algebra 2: logarithm properties, log equations | Classes, constructors, RAII | Read: RFC 9171 (BPv7) — the Bundle Protocol spec (pure protocol design) |
| 4 | Algebra 2: sequences, series, summation notation | Inheritance, polymorphism, virtual functions | DTN concepts: store-and-forward, custody transfer, convergence layers |
| 5 | Trigonometry: unit circle, sin/cos/tan | Templates basics, STL containers | **Elevation angle unlocked** — ground station visibility geometry |
| 6 | Trig: law of sines/cosines, identities | Iterators, algorithms, lambdas | Read: module 02 (CCSDS Protocol Stack) — it's all protocol architecture |
| 7 | Trig: polar coordinates, vectors intro | File I/O, binary data, bitwise ops | **🔨 Project: CCSDS Packet Parser (C++)** — parse binary protocol headers |
| 8 | Trig: angular velocity, arc length | Smart pointers, move semantics | Link budget intro: FSPL equation uses log₁₀ and trig (you now have both) |
| 9 | Trig review + problem sets | Error handling, exceptions, RAII patterns | **🔨 Project: dB Link Budget Calculator (C++)** — compute FSPL for LEO/GEO |
| 10 | Assessment: Algebra 2 + Trig mastery check | CMake, project structure, gtest | Read: module 07 (Ground Segment Architecture) — systems knowledge, no math |
...
## Phase 2: Acceleration (Weeks 11–22)
...
| Week | 🔢 Math (1.5 hr/day) | 💻 C++ (1 hr/day) | 🛰️ Space Networking (1.5 hr/day) |
|------|----------------------|--------------------|------------------------------------|
| 11 | Pre-calc: functions, composition, inverses | Networking: sockets (POSIX/Boost.Asio) | Read: RFC 5326 (LTP) — compare to TCP mentally |
| 12 | Pre-calc: exponential/log functions (deeper) | UDP client/server, async I/O | **🔨 Start: LTP Simulator (C++)** — red/green segments over UDP |
| 13 | Pre-calc: parametric equations, vectors | TCP client/server, protocol framing | LTP Simulator continued — checkpoint/retransmission |
| 14 | Pre-calc: conic sections (ellipses!) | Multithreading: std::thread, mutex | **Ellipse unlocked** → Kepler orbits are ellipses! Orbital shape intuition |
| 15 | Calculus I: limits, continuity | Concurrency: condition variables, atomics | Read: module 05 (LEO Constellations) — conceptual routing, topology |
| 16 | Calc I: derivatives — definition, rules | Serialization: CBOR (tinycbor), JSON | **Rate-of-change unlocked** → Doppler is derivative of range |
| 17 | Calc I: chain rule, implicit differentiation | CMake advanced, linking libraries | **🔨 Project: Doppler Curve Plotter (Python)** — derivatives in action |
| 18 | Calc I: applications — optimization, related rates | Boost.Graph intro | Read: module 10 (SDN) — you know SDN from terrestrial, concepts transfer |
| 19 | Calc I: integrals — fundamental theorem | Google Test, unit testing patterns | **Signal energy unlocked** → energy = ∫ power dt over a pass |
| 20 | Calc I: integration techniques | Profiling, performance optimization | Read: module 08 (DVB Standards) — system architecture (conceptual) |
| 21 | Calc I: applications of integration | Code review / refactor LTP Simulator | **🔨 Project: Contact Window Calculator (Python)** — when can a ground station see a satellite? Uses trig + calculus |
| 22 | Calc I mastery check + review | Polish and test all projects | Read: module 09 (5G NTN) — architecture overview (conceptual) |
...
─────────────────────────────────────────────────────────────────────────────
 7    Trig              CCSDS Packet Parser                   Binary I/O, bitwise
 9    Trig + Logs       dB Link Budget Calculator             Classes, CLI
12-13 Pre-calc          LTP Simulator                         UDP sockets, async
17    Calc I (deriv)    Doppler Curve Plotter (Python)         (Python + NumPy)
21    Calc I (integ)    Contact Window Calculator (Python)     (Python + Skyfield)
22    Calc I            DTN Node Prototype                    Serialization, threads
31-32 Calc III          Orbit Propagator                      Eigen, 3D math
37-38 Linear Algebra    Constellation Topology Engine         Spatial algorithms
43-44 Probability       Complete Link Budget Tool (Python)     (Python + SciPy)
47-48 Graph Theory      Contact Graph Router                  Boost.Graph, algorithms
51-52 All               CAPSTONE (pick 2 from Module 12)      Full stack
```
